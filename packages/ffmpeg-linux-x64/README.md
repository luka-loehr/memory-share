# @memory-share/ffmpeg-linux-x64

Static `ffmpeg` and `ffprobe` for `linux-x64`, installed as an optional
...
```bash
bun run packages/cli/scripts/vendor-ffmpeg.ts --platform linux-x64
```
