# @memory-share/ffmpeg-darwin-arm64

Static `ffmpeg` and `ffprobe` for `darwin-arm64`, installed as an optional
...
```bash
bun run packages/cli/scripts/vendor-ffmpeg.ts --platform darwin-arm64
```
